 ...
 * @author nhpj
 */
public class Test_Pirr_utan_ticket {
    @Test
    public void testNyttRecept() throws Exception {

        String endpoint         = "https://PirrHandler"; //
...
//        PirrHandler.setSoapEndpointUrl(endpoint + "/apisp-pi-wsgeneric-1.3/wsgeneric");
        pirr.setSoapEndpointUrl(endpoint + "/pi-wsgeneric-1.3/wsgeneric");

        pirr.setDATA(pirr.encode(recept.getXML())); // Signerar och Base64 encoda receptet

        System.out.println("150: Request\n" + pirr.getXML() + "\n : 150 - slut Request\n");
        ...
        System.out.println("160 Responsen: " + response.getXML());
        if ( !response.getTagValue("*//message").contains("ccepterat") ) { 
            ...
            else System.out.println("166: \n" + pirr.decode( response.getTagValue("*//data")));
            exit(1); 
        }
        System.out.println("169 data: " + response.getTagValue("*//data"));
        System.out.println("\n170: " + pirr.decode(response.getTagValue("*//data")));
        ...
        System.out.println("\n174: " + pirrRecept.getTagValue("*//Description") + " ( Läkare: " + LK_fname + " " + LK_ename + " kod: " + LK_kod + " Kategori: " + LK_kategori + ")");

        String retCode = pirrRecept.getTagValue("*//Description").substring(0, 1);
        if (!retCode.equals("0")) { 
            System.out.println("178: Stoppar på grund av fel! ");
            System.out.println("179: \n" + pirrRecept.getXML());
            exit(0); 
        }

        System.out.println("183: PrescriptionSetId: " + pirrRecept.getTagValue("*//PrescriptionSetId"));
        System.out.println("184: OrdinationsId: " + BaseXML.getOrdinationsId(pirrRecept.getTagValue("*//PrescriptionSetId"), "PTRR"));
